...
export default async function CalendarPage({
  ...
  const tasks = await getCalendarTasks(year, month);
  ...
  return (
    <div className="nc-app-shell">
      ...
      <div style={{ flex: 1, display: "flex", flexDirection: "column", minWidth: 0 }}>
        <TopBar title="Calendario" subtitle={`${MONTHS[month - 1]} ${year}`} />
        ...
        >
          <Link href={`/calendario?y=${prevMonth.y}&m=${prevMonth.m}`} className="nc-icon-btn" aria-label="Mes anterior">
            <IChev dir="left" size={13} />
          </Link>
          <div style={{ fontSize: 13, fontWeight: 600, minWidth: 140 }}>
            {MONTHS[month - 1]} {year}
          </div>
          <Link href={`/calendario?y=${nextMonth.y}&m=${nextMonth.m}`} className="nc-icon-btn" aria-label="Mes siguiente">
          ...
          <div style={{ fontSize: 11, color: "var(--nc-mute)" }}>
            {tasks.length} tarea{tasks.length !== 1 ? "s" : ""} con fecha este mes
          </div>
        </div>

        {/* Calendar grid */}
        <div style={{ flex: 1, display: "grid", gridTemplateRows: "auto 1fr", background: "var(--nc-bg)", minHeight: 0 }}>
          {/* Days-of-week header */}
          <div
            style={{
              display: "grid",
